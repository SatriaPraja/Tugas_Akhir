    ...
    <div class="p-4 md:p-8">
        ...
        <div class="flex flex-col lg:flex-row lg:justify-between lg:items-end mb-8 space-y-6 lg:space-y-0">
            <div>
                ...
                <p class="text-gray-500 mt-1 text-sm md:text-base">Kelola informasi klasterisasi, produktivitas, dan
                    koordinat lahan.</p>
            </div>
            ...
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center space-y-3 sm:space-y-0 sm:space-x-3">
                ...
                <form action="{{ request()->url() }}" method="GET" class="flex flex-1 items-center space-x-2">
                    ...
                    @if (request('search') || request('filter_klaster'))
                        <a href="{{ request()->url() }}"
                            class="p-2.5 bg-blue-600 text-white rounded-xl transition-all shadow-sm flex items-center justify-center hover:bg-blue-700"
                            title="Reset">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50/50">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-bold text-blue-900 uppercase tracking-wider">Est.
                                Panen</th>
                            <th class="px-6 py-4 text-left text-xs font-bold text-blue-900 uppercase tracking-wider">Pupuk
                            </th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        @forelse ($lahan as $item)
                            <tr class="hover:bg-blue-50/30 transition-colors duration-200">
                                ...
                                <td class="px-6 py-4 text-sm text-blue-600 font-bold whitespace-nowrap">
                                    {{ $item->produktivitas ?? 0 }} <span
                                        class="text-[10px] text-gray-400 font-normal uppercase tracking-tighter">Kg/Ha</span>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-600 whitespace-nowrap">
                                    {{ number_format($item->estimasi_panen) }} <span
                                        class="text-xs text-gray-400">Kg</span>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    @endphp
                                    <span class="px-3 py-1 rounded-full text-[11px] font-bold text-white shadow-sm"
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="px-6 py-12 text-center text-gray-500">Data lahan tidak
                                    ditemukan.</td>
                            </tr>
                        @endforelse
                    </tbody>
                ...
                <div class="p-6 space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div class="space-y-1">
                            ...
                            <input type="text" name="nop" required placeholder="32.01..."
                                class="block w-full border-2 border-gray-200 rounded-xl focus:border-green-500 text-sm py-2.5 px-4 outline-none transition-all">
                        </div>
                        <div class="space-y-1">
                            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">Nama
                                Pemilik</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div class="space-y-1">
                            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">Luas Lahan
                                (m²)</label>
                            <input type="number" name="luas" required min="1"
                                class="block w-full border-2 border-gray-200 rounded-xl focus:border-green-500 text-sm py-2.5 px-4 outline-none transition-all">
                        </div>
                        <div class="space-y-1">
                            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">Jenis
                                Tanah</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 items-center">
                        <div class="space-y-1">
                            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">Est. Hasil
                                Panen (Kg)</label>
                            <input type="number" name="estimasi_panen" required
                                class="block w-full border-2 border-gray-200 rounded-xl focus:border-green-500 text-sm py-2.5 px-4 font-bold text-green-700 outline-none transition-all">
                        </div>
                        <div class="bg-green-50 p-4 rounded-xl border border-dashed border-green-200">
                            <div class="text-[10px] font-bold text-green-600 uppercase tracking-widest">Auto-Kalkulasi
                            </div>
                            <div class="text-xs font-semibold text-green-800 mt-1 leading-tight">Urea, NPK, & Produktivitas
                                dihitung otomatis saat simpan.</div>
                        </div>
                    </div>
                </div>
                <div
                    class="bg-gray-50 px-6 py-4 flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3">
                    <button type="button" onclick="closeCreateModal()"
                        class="px-6 py-2.5 bg-white border border-gray-200 text-gray-600 text-sm font-bold rounded-xl hover:bg-gray-50">Batal</button>
                    <button type="submit"
                        class="px-10 py-2.5 bg-green-600 text-white text-sm font-black rounded-xl hover:bg-green-700 transition-all uppercase tracking-widest border-b-4 border-green-800">Simpan
                        Data</button>
                </div>
            </form>
        </div>
